...
function isValidTableBlock(block: any): boolean {
  ...
  return (
    ...
    block.rows.every(
      (row: any) => Array.isArray(row) && row.length === block.headers.length
    )
  )
}

export function parseWhiteboardBlocks(
  raw: string
): WhiteboardContentBlock[] | null {
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (!validation.success) {
          console.error(
            "❌ [VALIDATION FAIL] Invalid Whiteboard JSON:",
        ...
        if (validItem.render === "chemistry") {
          ...
          validItem.render === "graph" ||
          validItem.render === "chart"
        ) {
        ...
        if (validItem.render === "math") {
          ...
          const content = [
            ...
            validItem.content.law
              ? `\\textit{Law: ${validItem.content.law}}`
              : "",
            ...(Array.isArray(validItem.content.latex)
              ? validItem.content.latex
              : [validItem.content.latex || ""]),
            validItem.content.final_answer
              ? `\\boxed{${validItem.content.final_answer}}`
              : ""
          ]
            .filter(Boolean)
            .join("\n\n")
      ...
      if (blocks.length > 2) {
        console.warn(
          `⚠️ [SAFETY] Capping whiteboard blocks from ${blocks.length} to 2.`
        )
  ...
  if (wbMatch) {
    try {
      ...
      if (Array.isArray(parsed.blocks)) {
        const valid = parsed.blocks.filter((b: any) =>
          [
            "latex",
            "table",
            "canvas",
