...
export function validateChemistryBlock(
  block: Extract<WhiteboardResponse, { render: "chemistry" }>
): boolean {
  ...
  for (const mol of allMolecules) {
    if (mol.name.toLowerCase().includes("benzene")) {
      ...
      if (mol.smiles.trim().length < 3 || mol.smiles === "C6H6") {
        console.warn(
          `⚠️ [SEMANTIC GUARD] Invalid SMILES for Benzene: ${mol.smiles}`
        )
        return false
      }
    }
...
export function validateGraphBlock(
  block: Extract<WhiteboardResponse, { render: "graph" | "chart" }>
): boolean {
  const content = block.content
  ...
  if (
    !content.domain ||
    !Array.isArray(content.domain) ||
    content.domain.length !== 2
  ) {
    console.warn(
      `⚠️ [SEMANTIC GUARD] Invalid domain for graph: ${JSON.stringify(content.domain)}`
    )
    return false
  }
