...
export const filesystemMcp = {
  readPDF: (filePath: string): Buffer => {
    ...
    const isNcert = absolutePath.startsWith(
      path.resolve(process.cwd(), "data/ncert_pdfs")
    )
    const isGuide = absolutePath.startsWith(
      path.resolve(process.cwd(), "data/guide_pdfs")
    )

    if (!isNcert && !isGuide) {
      throw new Error(
        `Security Violation: Accessed path outside approved data directories`
      )
    }
