...
const N8N_WEBHOOK_URL = process.env.NEXT_PUBLIC_N8N_WEBHOOK_URL || 'http://localhost:5678/webhook/ai-tutor';
...
export async function sendToN8n(
    ...
    const { mode = 'tutor', forceWhiteboard = false, allowWhiteboard = true } = options;
...
export function fallbackParse(
    ...
    const drawKeywords = ['draw', 'show', 'illustrate', 'visualize', 'diagram', 'sketch'];
...
function generateSampleDrawing(userMessage: string): AIResponse {
    ...
    // Coordinate axes
    if (content.includes('axis') || content.includes('axes') || content.includes('graph')) {
        return {
            text: '',
            whiteboard_action: 'draw',
            commands: [
    ...
    return {
        ...
        whiteboard_action: 'draw',
        commands: [
            { type: 'label', text: 'AI Tutor Whiteboard', position: [280, 50], fontSize: 24 },
            { type: 'line', from: [200, 100], to: [600, 100] },
            { type: 'label', text: 'Request: ' + userMessage.slice(0, 50), position: [150, 150] },
