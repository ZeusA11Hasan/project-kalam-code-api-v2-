...
export function useTTS() {
    ...
    const speakText = useCallback(async (text: string, languageCode?: string) => {
        ...
        console.log(`[TTS Hook] speakText called — ${chunks.length} chunk(s), total ${text.length} chars`);
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            if (stoppedRef.current) {
                console.log("[TTS Hook] Playback stopped by user, aborting remaining chunks");
                break;
            }

            const chunk = chunks[i];
            console.log(`[TTS Hook] Playing chunk ${i + 1}/${chunks.length}: "${chunk.slice(0, 40)}..."`);
            ...
            try {
                await new Promise<void>((resolve) => {
                    fetch("/api/tts", {
                        ...
                        .then(res => {
                            if (!res.ok) {
                                console.error(`[TTS Hook] Chunk ${i + 1} fetch error: ${res.status}`);
                                resolve();
                                return null;
                            }
                            return res.blob();
                        ...
                        .then(blob => {
                            ...
                            audio.onerror = () => {
                                console.error(`[TTS Hook] Chunk ${i + 1} audio playback error`);
                                URL.revokeObjectURL(url);
                                setActiveAudioElement(null);
                                resolve();
                            };
