...
export function useWebSpeech(
        ...
        audioCheckIntervalRef.current = setInterval(() => {
          ...
          if (average < 10) {
            ...
            if (silenceCountRef.current >= 10) {
              console.log(
                "[WebSpeech Hook] Auto-stopping due to 2 seconds of silence"
              )
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      if (navigator.mediaDevices?.getUserMedia) {
        ...
        mediaRecorder.onstop = async () => {
          ...
          try {
            ...
            console.error(
              "[WebSpeech] ❌ Sarvam STT error, using Web Speech fallback:",
              err
            )
            if (fallbackTranscriptRef.current) {
              ...
              onError?.(
                "Speech recognition failed on both Sarvam and fallback APIs."
              )
            }
          }
        }
        ...
        stopTimerRef.current = setTimeout(() => {
          console.log(
            "[WebSpeech Hook] 5-second max duration reached. Auto-stopping."
          )
          stopListening()
        }, 5000)
      }
      ...
      console.warn(
        "[WebSpeech] ⚠️ MediaRecorder start failed, relying entirely on Web Speech fallback.",
        mediaErr
      )
    }
    ...
    recognition.onstart = () => {
      console.log(
        "[WebSpeech] 🎤 Web Speech Listening started (Interim/Fallback mode)"
      )
    }
