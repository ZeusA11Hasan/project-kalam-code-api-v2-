  ...
  switch (block.type) {
          ...
          <table className="w-full border-collapse text-left text-sm text-zinc-200">
            ...
            <tbody>
              {block.rows.map((row, i) => (
                <tr
                  key={i}
                  className="border-b border-zinc-600 hover:bg-zinc-800/20"
                >
                  {row.map((cell, j) => (
    ...
    case "chemistry":
      ...
      if (molecules.length === 0 && block.content.structure) {
        molecules.push({
          name: block.content.label || "Molecule",
          smiles: block.content.structure
        })
      }
          ...
          <ChartRenderer
            element={{
              ...
              data: block.content.data
                ? { datasets: [{ data: block.content.data }] }
                : undefined
            }}
