...
const PremiumReviewButton = ({
    ...
    return (
        ...
        >
            ...
            <div className={`pointer-events-none absolute inset-0 rounded-xl transition-opacity duration-500 ${isActive ? 'bg-[#8A9A5B]/10 opacity-100' : 'bg-transparent opacity-0'}`} />
...
const PremiumToolbarButton = ({
    ...
    return (
        ...
        >
            ...
            <div className={`pointer-events-none absolute inset-0 rounded-full transition-colors ${isSuccess ? 'bg-white/0 group-hover:bg-white/[0.03]' : 'bg-white/0 group-hover:bg-white/[0.03]'}`} />
        </motion.button>
    );
};
                    ...
                    {data?.values?.map((row, rowIdx) => (
                        ...
                        >
                            {row.map((cell, cellIdx) => (
                                <td key={cellIdx} className="px-4 py-3 text-[13px] font-medium text-white/70">
                                    {String(cell)}
                                </td>
                            ))}
                        </motion.tr>
...
export default function PythonIDE({
    ...
    const handleInputSubmit = () => {
        if (inputResolverRef.current) {
            ...
            setOutput(prev => [...prev, { type: 'text', content: inputPrompt + value }]);
        ...
        try {
            if (language === 'python') {
                if (!window.pyodideInstance) {
                    ...
                    (window as any).__pyodideRequestInput = (prompt: string): Promise<string> => {
...
sys.stdout = io.StringIO()
                ...
                try {
                    ...
                    if (/\binput\s*\(/.test(code)) {
                        const transformed = code.replace(/\binput\s*\(/g, 'await __async_input(');
                    ...
                    const remaining = await pyodide.runPythonAsync("sys.stdout.getvalue()");
                    ...
                    setOutput(prev =>
                        prev.length === 0
                            ? [{ type: 'text', content: "> Program execution finished (no output)." }]
                            : prev
                    );
                } catch (err: any) {
                    ...
                    if (errorMsg.includes("'await' outside") || errorMsg.includes("await")) {
                        // Check if it's the async function limitation
                        if (errorMsg.includes("non-async") || errorMsg.includes("outside function")) {
                            errorMsg += "\n\n\ud83d\udca1 Tip: input() inside nested def functions is not yet supported. Move input() to the top level of your code.";
                        }
                    }
                    setOutput(prev => [...prev, { type: 'error', content: "> Python Execution Error:\n" + errorMsg }]);
                }
            } else {
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-white">Output Console</span>
                </div>
                <div className="whitespace-pre-wrap text-emerald-400/90">
                    ...
                    {isWaitingForInput && (
                        ...
                        >
                            <span className="select-none text-[12px] text-white/20">$</span>
                            <span className="whitespace-pre text-[14px] text-yellow-400/80">{inputPrompt}</span>
                            <input
                                ref={inputFieldRef}
                                type="text"
                                value={inputValue}
