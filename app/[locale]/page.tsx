...
export default function HomePage() {
    ...
    // In call mode — always default to Tamil (rural students)
    // Only switch to Hindi if explicitly detected
    const resolvedLanguage = (langCode: string): string => {
        ...
        // Default for call mode = Tamil (rural students)
        return "ta-IN"; // Changed from en-IN to ta-IN
    };
    ...
    const webSpeech = useWebSpeech({
        ...
        onFinalTranscript: (text, languageCode) => {
            ...
            if (text.trim()) {
                ...
                handleSend(text.trim(), languageCode ? resolvedLanguage(languageCode) : undefined);
            }
        },
        onError: (error) => {
    ...
    const handleSend = async (message: string, filesOrLangCode?: File[] | string) => {
        if (!user) {
            router.push('/login');
            return;
        }
        ...
        const overrideLangCode = typeof filesOrLangCode === 'string' ? filesOrLangCode : undefined;
        const langToPass = overrideLangCode || detectedLanguage;
        await processAICall([...messages, newMessage], langToPass);
    };

    const processAICall = async (currentMessages: Message[], langCode?: string) => {
        setIsAILoading(true);
        ...
        try {
            ...
            console.log("[DEBUG] Full response data:", JSON.stringify(data).slice(0, 300));
            console.log("[DEBUG] data.explanation:", typeof data.explanation, data.explanation?.slice?.(0, 100));
            const rawAiContent = data.explanation || data.reply || 'I received your message.';

            // Strip <think> tags (model reasoning) before showing in UI
            let strippedContent = rawAiContent.replace(/<think>[\s\S]*?<\/think>/g, "");
            // Only strip unclosed <think> if no </think> follows
            if (strippedContent.includes("<think>") && !strippedContent.includes("</think>")) {
                strippedContent = strippedContent.replace(/<think>[\s\S]*$/g, "");
            }
            strippedContent = strippedContent.trim();
            // Safety: never let stripping produce empty string
            const aiContent = strippedContent || rawAiContent.replace(/<\/?think>/g, "").trim() || 'I received your message.';
            ...
            if (isCallActiveRef.current || voiceModeEnabled) {
                ...
                const speakLangCode = finalLang.startsWith("hi")
                    ? "hi-IN"
                    : "ta-IN"; // Always Tamil unless Hindi
                ...
                console.log("[TTS] isCallActiveRef:", isCallActiveRef.current, "voiceModeEnabled:", voiceModeEnabled);
                console.log("[TTS] Language:", speakLangCode);
                console.log("[TTS] Text preview:", plainText.slice(0, 60));
            ...
            console.log("[IDE] Code received:", data.code ? data.code.slice(0, 80) : "(empty)");
            ...
            <AnimatePresence mode="wait">
                {!isChatMode && (
                    ...
                    >
                        ...
                        <div className="pointer-events-none fixed inset-0 z-10 bg-black/20 backdrop-blur-xl" />
                        ...
                        <div className="flex flex-1 flex-col items-center justify-center gap-6 px-6 pb-12 pt-[15vh] md:gap-10 md:px-20 md:pt-[18vh]">
                            ...
                            >
                                ...
                                <h1 className="font-darker-grotesque mb-2 text-[75px] font-bold leading-[0.9] tracking-tight text-white md:text-[125px]">
                                    {getGreeting()}, <span className="animate-gradient font-darker-grotesque bg-gradient-to-r from-blue-400 via-cyan-300 to-blue-400 bg-[length:200%_auto] bg-clip-text text-transparent">{studentName}</span>
                                </h1>
                            </motion.div>
                    ...
                    >
                        ...
                        <div className="relative flex size-full max-h-[98vh] gap-0 md:h-[98vh]">
                            ...
                            >
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="relative size-10 overflow-hidden rounded-xl border border-cyan-500/30 shadow-[0_0_15px_rgba(6,182,212,0.1)]">
                                            <img src="/max-avatar.png" alt="Profile" className="size-full object-cover" />
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-[14px] font-bold leading-tight text-white">Hey, {studentName}</span>
                                ...
                                >
                                    <ScrollArea className="h-full px-4 py-8 md:px-10">
                                            ...
                                            {messages.length === 0 ? (
                                                ...
                                                <>
                                                    {messages.map((message, index) => (
                                                        <motion.div key={message.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className={`flex w-full flex-col ${message.role === "user" ? "items-end" : "items-start"}`}>
                                                            <div className={`group flex max-w-full gap-4 ${message.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                                                                <div className={`flex flex-col ${message.role === "user" ? "items-end" : "items-start"} min-w-0 max-w-[85%]`}>
                                                                    <div className={cn(
                                                                        "font-outfit w-full max-w-full overflow-hidden break-words py-1 leading-relaxed tracking-tight",
                                                                        message.role === "user"
                                                                    ...
                                                                    <div className={`mt-2 flex gap-3 pl-1 text-white/20 opacity-0 transition-opacity group-hover:opacity-100`}>
                                                                        <button onClick={() => copyToClipboard(message.content)} className="transition-colors hover:text-white">
                                                                            <Copy className="size-4" />
                                                                        </button>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            {index < messages.length - 1 && <div className="my-8 h-px w-full bg-white/[0.04]" />}
                                                        </motion.div>
                                                    ))}
                                                    {isAILoading && (
                                                            ...
                                                            <div className="rounded-2xl border border-white/10 bg-black/60 px-6 py-4 backdrop-blur-xl">
                                                                <div className="flex items-center gap-3">
                                                                    <div className="flex gap-1.5">
                                                                        ...
                                                                        <div className="size-2 animate-pulse rounded-full bg-cyan-400/60" style={{ animationDelay: '0.2s' }} />
                                                                        <div className="size-2 animate-pulse rounded-full bg-cyan-400/40" style={{ animationDelay: '0.4s' }} />
                                                                    </div>
                                                                    <span className="font-outfit text-[12px] font-semibold uppercase tracking-widest text-white/70">Agent is thinking...</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    )}
                                ...
                                <div
                                    className="shrink-0 px-4 pb-4 pt-2 transition-all duration-500 md:px-8 md:pb-6"
                                    style={{
                                        background: isCallActive || !isWhiteboardOpen ? 'transparent' : 'linear-gradient(145deg, rgba(14, 14, 16, 0.95), rgba(10, 10, 12, 0.95))',
                                        boxShadow: isCallActive || !isWhiteboardOpen ? 'none' : 'inset -2px -2px 6px rgba(255,255,255,0.008), inset 3px 3px 8px rgba(0,0,0,0.6)',
                                    }}
                                >
                            ...
                            <AnimatePresence>
                                {isWhiteboardOpen && (
                                    <motion.div
                                        ...
                                        className={`group relative z-[60] hidden w-3 shrink-0 cursor-col-resize items-center justify-center md:flex`}
                                        onMouseDown={(e) => { e.preventDefault(); setIsResizing(true); }}
                                    >
                                        <div className={`h-16 w-1 rounded-full transition-all duration-300 ${isResizing ? 'scale-x-150 bg-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.8)]' : 'bg-white/10 group-hover:bg-white/40'}`} />

                                        {/* Invisible wide hit area */}
                                        <div className="absolute -inset-x-2 inset-y-0 bg-transparent" />
                                    </motion.div>
                                )}
                            </AnimatePresence>
