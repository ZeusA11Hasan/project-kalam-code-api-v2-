...
const dockItems = [
  ...
  {
    title: "Library",
    icon: <BookOpen className="size-full text-white" />,
    href: "/library"
  },
  {
    title: "Profile",
...
export default function HomePage() {
  ...
  const processAICall = async (
    ...
    try {
      ...
      const rawAiContent =
        data.explanation || data.reply || "I received your message."

      // Strip <think> tags (model reasoning) before showing in UI
      let strippedContent = rawAiContent.replace(
        /<think>[\s\S]*?<\/think>/g,
        ""
      )
      // Only strip unclosed <think> if no </think> follows
      if (
        strippedContent.includes("<think>") &&
        !strippedContent.includes("</think>")
      ) {
      ...
      console.log(
        "[IDE] Code received:",
        data.code ? data.code.slice(0, 80) : "(empty)"
      )
            ...
            <div className="relative flex size-full max-h-[98vh] gap-0 md:h-[98vh]">
              ...
              >
                        ...
                        <>
                          {messages.map((message, index) => (
                            ...
                            >
                              ...
                              >
                                <div
                                  className={`flex flex-col ${message.role === "user" ? "items-end" : "items-start"} min-w-0 max-w-[85%]`}
                                >
                                  <div
                                    className={cn(
                ...
                <div
                  ...
                  style={{
                    background:
                      isCallActive || !isWhiteboardOpen
                        ? "transparent"
                        : "linear-gradient(145deg, rgba(14, 14, 16, 0.95), rgba(10, 10, 12, 0.95))",
                    boxShadow:
                      isCallActive || !isWhiteboardOpen
                        ? "none"
                        : "inset -2px -2px 6px rgba(255,255,255,0.008), inset 3px 3px 8px rgba(0,0,0,0.6)"
              ...
              <AnimatePresence>
                {isWhiteboardOpen && (
                  ...
                  >
                    <div
                      className={`h-16 w-1 rounded-full transition-all duration-300 ${isResizing ? "scale-x-150 bg-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.8)]" : "bg-white/10 group-hover:bg-white/40"}`}
                    />
