...
export function parseCanvasBlock(raw: any): WhiteboardData | null {
    try {
        ...
        if (parsed.type === "structure" || parsed.type === "reaction" || parsed.type === "mechanism") {
        ...
        for (const step of steps) {
            if (typeof step.stepId !== "number" || !Array.isArray(step.elements)) continue;
