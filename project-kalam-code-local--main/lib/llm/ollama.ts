...
async function checkModelAvailability(modelName: string): Promise<boolean> {
    try {
        ...
        console.log(`[OLLAMA] Model Verification: ${modelName} -> ${exists ? "FOUND" : "MISSING"}`);
        return exists;
    } catch (error: any) {
...
export async function callOllama(
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            throw new Error("Ollama request timed out after 300s (5 min). Suggestion: Preload model using 'docker exec -it ollama ollama run " + model + "'");
        }
        throw error;
    }
}
