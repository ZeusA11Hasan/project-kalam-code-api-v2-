...
export async function POST(req: NextRequest) {
    try {
        ...
        await execPromise(`edge-tts --voice ${voice} -f "${tempTextFile}" --write-media "${tempMediaFile}"`);
        ...
        return NextResponse.json({ error: "Edge TTS failed: " + error.message }, { status: 500 });
    }
}
