...
export const ChatMorphBar = React.forwardRef<HTMLDivElement, ChatMorphBarProps>((props, ref) => {
    const {
        onSend = () => { },
        onStop,
        onStartCall,
    ...
    useGSAP(() => {
        ...
        if (isCallActive) {
            ...
            if (callDockRef.current) {
                gsap.set(callDockRef.current, { display: "flex", scale: 0.9, y: 20, opacity: 0, filter: "blur(5px)" });
                tl.to(callDockRef.current, {
                    scale: 1,
                    y: 0,
                    opacity: 1,
                    filter: "blur(0px)",
            ...
            if (callDockRef.current) {
                tl.to(callDockRef.current, {
                    ...
                    ease: "power2.inOut",
                    onComplete: () => {
                        if (callDockRef.current) callDockRef.current.style.display = "none";
                    }
                }, 0);
            }
            if (chatRef.current) {
                if (initialRender.current) {
                    gsap.set(chatRef.current, { display: "flex", scale: 1, opacity: 1, filter: "blur(0px)" });
                } else {
                    gsap.set(chatRef.current, { display: "flex", scale: 0.95, opacity: 0, filter: "blur(5px)" });
                    tl.to(chatRef.current, {
                        scale: 1,
                        opacity: 1,
                        filter: "blur(0px)",
    ...
    // ======================================================================
    return (
        <div ref={ref} className={cn("w-full mx-auto relative", className)}>
            <LayoutGroup>
                {/* ====================================================
                    CHAT INPUT MODE
                 ==================================================== */}
                <div
                    ...
                    style={{
                        background: "linear-gradient(145deg, rgba(13,13,15,0.92), rgba(8,8,10,0.95))",
                ...
                {isMounted && createPortal(
                    <div className="fixed inset-x-0 bottom-[40px] z-[9999] pointer-events-none flex items-end justify-center">
                        <div
                            ref={callDockRef}
                            className="pointer-events-auto flex items-center justify-center gap-5 px-6 py-3 rounded-[3rem] w-max max-w-[95vw] sm:max-w-fit relative"
                            style={{
                                background: "linear-gradient(145deg, rgba(13,13,15,0.95), rgba(8,8,10,0.98))",
                                boxShadow: [
                        ...
                        >
                                ...
                                <motion.button
                                    ...
                                    className={cn(
                                        ...
                                        isListening
                                            ...
                                            : "text-red-500 bg-[#0d0d0f] shadow-[-5px_-5px_12px_rgba(255,255,255,0.04),6px_6px_16px_rgba(0,0,0,0.9)]"
                                    )}
                                >
                                    {isListening ? <Mic size={20} strokeWidth={2.5} /> : <MicOff size={20} strokeWidth={2.5} />}
                                </motion.button>
                                <span className={cn("text-[8px] font-black uppercase tracking-[0.2em]", isListening ? "text-white/25" : "text-red-500/70")}>
                                ...
                                {(isLoading || isSpeaking) && (
                                    ...
                                    >
                                        <motion.button
                                            ...
                                            whileTap={{ scale: 0.93 }}
                                            onClick={() => onStop?.()}
                                            className="flex items-center justify-center size-12 rounded-full bg-[#0d0d0f] shadow-[-5px_-5px_12px_rgba(255,255,255,0.03),6px_6px_16px_rgba(0,0,0,0.9)] text-red-500"
                                        >
                            ...
                            <div className="flex flex-col items-center gap-1 z-10">
                                <LiveWaveform isActive={isListening || isSpeaking} isSpeaking={isSpeaking} />
                                <span className={cn(
                                    "text-[8px] font-black uppercase tracking-[0.2em]",
                                    isSpeaking ? "text-cyan-400/60" : isListening ? "text-white/25" : "text-white/15"
                                )}>
                            ...
                            <motion.button
                                ...
                                whileTap={{ scale: 0.93 }}
                                onClick={onEndCall}
                                className="relative flex items-center justify-center w-[4.5rem] h-11 rounded-[1.5rem] bg-[#FF3B30] text-white z-10 mx-16"
                            >
                                <Phone size={22} fill="currentColor" className="relative rotate-[135deg]" />
                            </motion.button>
                            ...
                            <div className="flex flex-col items-center gap-1 z-10">
                                <motion.button
                                    whileHover={{ scale: 1.08 }}
                                    whileTap={{ scale: 0.93 }}
                                    className="flex items-center justify-center size-12 rounded-full bg-[#0d0d0f] shadow-[-5px_-5px_12px_rgba(255,255,255,0.025),6px_6px_14px_rgba(0,0,0,0.9)] text-white/35 hover:text-white/70 transition-colors"
                                >
                                    <Users size={20} strokeWidth={2.5} />
                                </motion.button>
                                <span className="text-[8px] font-black uppercase tracking-[0.2em] text-white/20">Call</span>
                            </div>

                            {/* Border accent */}
                            <div className="absolute inset-0 rounded-[3rem] border border-white/[0.04] pointer-events-none" />
                        </div>
                    </div>,
                    document.body
                )}
