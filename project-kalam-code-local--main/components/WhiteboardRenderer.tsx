...
export default function WhiteboardRenderer({ blocks, onSpeak, onStopSpeak, isVoiceMode }: Props) {
    console.log("🎨 WhiteboardRenderer PRE-RENDER", { FORCE_DEMO_MODE, blocksLength: blocks?.length });
    ...
    return (
        <div className="relative w-full h-full overflow-hidden bg-[#0a0a0a]">
            {/* 🐛 DEBUG OVERLAY */}
            <div className="absolute top-20 left-20 z-50 text-red-500 font-bold bg-black/50 p-2 border border-red-500">
            ...
            <InfiniteCanvas>
                ...
                {visibleElements.map((el, i) => (
                    <RenderElement key={`${currentStepIndex}-${i}`} element={el} index={i} />
                ))}
            </InfiniteCanvas>

            {/* 🎮 PLAYBACK CONTROLS (Bottom Center Overlay) */}
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 p-2 bg-[#1e1e1e]/90 backdrop-blur border border-white/10 rounded-xl shadow-2xl z-50 pointer-events-auto">
                <ControlButton icon={<RotateCcw size={16} />} onClick={handleReset} label="Restart" />
                <div className="w-px h-4 bg-white/10 mx-1" />
                <ControlButton icon={<SkipBack size={16} />} onClick={handlePrev} label="Prev" disabled={currentStepIndex === 0} />
                <ControlButton
                    ...
                    active={isPlaying}
                />
                <ControlButton icon={<SkipForward size={16} />} onClick={handleNext} label="Next" disabled={currentStepIndex === sequence.length - 1} />
            ...
            <motion.div
                ...
                className="absolute top-8 right-8 w-80 pointer-events-auto z-40"
            >
                <div className="bg-[#1e1e1e]/90 backdrop-blur border border-white/10 p-5 rounded-xl shadow-2xl">
                    <h3 className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-3 flex justify-between items-center">
                        <span>Step {currentStepIndex + 1} / {sequence.length}</span>
                        {isPlaying && <span className="text-blue-400 text-[10px] animate-pulse">● AUTO-PLAY</span>}
                    </h3>
                    ...
                    <AnimatePresence mode="wait">
                        <motion.div
                            ...
                            exit={{ opacity: 0, x: -10 }}
                            className="text-white font-medium text-lg leading-snug"
                        >
                            {currentExplanation}
                        </motion.div>
                    </AnimatePresence>
                    ...
                    <div className="w-full h-1 bg-white/10 mt-4 rounded-full overflow-hidden">
                        <motion.div
                            ...
                            style={{ width: `${((currentStepIndex + 1) / sequence.length) * 100}%` }}
                        />
                    </div>
                </div>
            </motion.div>
...
function ControlButton({ icon, onClick, label, active = false, disabled = false }: any) {
    return (
        <button
            onClick={onClick}
            disabled={disabled}
            className={`p-2 rounded-lg transition-all duration-200 group relative
                ${active ? "bg-blue-500 text-white" : "hover:bg-white/10 text-gray-400 hover:text-white"}
...
function RenderElement({ element, index }: { element: CanvasElement; index: number }) {
    ...
    const getPos = (el: any) => {
        ...
        if (el.zone || (el.row !== undefined && el.col !== undefined)) {
            ...
            const zoneX = ZONE_OFFSETS[zone as keyof typeof ZONE_OFFSETS] || ZONE_OFFSETS.CENTER;
            return {
        ...
        if ('at' in el) {
            return { x: ORIGIN_X + el.at[0] * LEGACY_SCALE, y: ORIGIN_Y - el.at[1] * LEGACY_SCALE };
        }
    ...
    switch (element.type) {
        case "box": {
            ...
            return (
                <div
                    className="absolute border border-blue-400/50 bg-blue-500/10 rounded-lg flex items-center justify-center text-center p-2 backdrop-blur-sm z-10 overflow-hidden"
                    style={{
                        left: pos.x,
                        top: pos.y,
                        width: `${width}px`,
                ...
                >
                    <span className="text-blue-100 font-medium text-xs md:text-sm leading-tight">
        ...
        case "text": {
            return (
                <motion.div
                    ...
                    className="absolute text-white/90 font-handwriting text-lg whitespace-pre z-20"
        ...
        case "label": {
            return (
                <motion.div
                    drag
                    className="absolute text-white text-sm z-20 bg-black/50 px-2 rounded"
...
function CooldownTimer({ element }: { element: any }) {
    ...
    return (
        ...
        >
            <div className="flex items-center gap-3 bg-red-500/10 border border-red-500/50 backdrop-blur-md px-6 py-3 rounded-full shadow-lg shadow-red-500/10">
                <div className="relative flex items-center justify-center w-5 h-5">
                    <div className="absolute w-full h-full border-2 border-red-500 rounded-full opacity-30 animate-ping" />
                    <div className="w-2 h-2 bg-red-500 rounded-full" />
                </div>
                <span className="text-red-200 font-mono font-bold tracking-widest text-lg">
