    ...
    return (
        ...
        >
            ...
            {svgElements.map((element) => {
                switch (element.type) {
                    case 'text':
                        return <TextSvgElement key={element.id} element={element} viewport={viewport} />
                    case 'arrow':
                        return <ArrowSvgElement key={element.id} element={element} viewport={viewport} />
                    case 'line':
                        return <LineSvgElement key={element.id} element={element} viewport={viewport} />
                    case 'shape':
                        return <ShapeSvgElement key={element.id} element={element} viewport={viewport} />
                    default:
                        return null
                }
            })}
...
function ArrowSvgElement({
    ...
    const markerId = useMemo(() => {
        if (color.includes('blue') || color === '#3b82f6') return 'url(#arrowhead-blue)'
        if (color.includes('red') || color === '#ef4444') return 'url(#arrowhead-red)'
