...
import type { ChemCanvasPayload, ChemStructure, ChemReaction, ChemMechanism, BondChange } from "./chemistryTypes";
    ...
    return (
        <div className="relative w-full h-full">
            ...
            {payload?.type === "mechanism" && (
                <div className="absolute bottom-4 left-4 bg-black/50 px-3 py-1 rounded text-xs text-white/70 pointer-events-none">
...
function highlightBond(ctx: CanvasRenderingContext2D, s: ChemStructure, change: BondChange) {
    const scale = 60;
    const ox = 400;
    const oy = 250;
...
function drawDoubleBond(ctx: CanvasRenderingContext2D, x1: number, y1: number, x2: number, y2: number) {
    const dx = y2 - y1;
    const dy = x1 - x2;
    const norm = Math.sqrt(dx * dx + dy * dy) || 1;
    const off = 4; // Gap offset

    drawLine(ctx, x1 + (dx / norm) * off, y1 + (dy / norm) * off, x2 + (dx / norm) * off, y2 + (dy / norm) * off);
    drawLine(ctx, x1 - (dx / norm) * off, y1 - (dy / norm) * off, x2 - (dx / norm) * off, y2 - (dy / norm) * off);
}

function drawTripleBond(ctx: CanvasRenderingContext2D, x1: number, y1: number, x2: number, y2: number) {
